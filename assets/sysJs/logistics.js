		...
		.then(function(response) {
			...
			if (data.length !== 0) {
				data.map((item, indx) => {
					...
					res += `<td>${item.resource}</td>`;
					res += `<td>${item.quantity_of_resources_used}</td>`;
					res += `<td>${moment(item.creationDate, 'YYYY-MM-DD').format('LL')}</td>`;
					res += `<td>${item.action_start_time}</td>`;
					res += `<td>${item.action_end_time}</td>`;
